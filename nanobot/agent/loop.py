...
class _LoopHook(AgentHook):
    """
    整个loop对应的核心的钩子，继承了AgentHook，负责在代理循环的不同阶段插入自定义逻辑。
    """
    ...
    def __init__(
        ...
        *,
        ...
        # 注意：这个类的实例会在每次调用agent_loop._run_agent_loop时创建，因此它不应该持有任何跨迭代的状态。
        # 把agent_loop作为一个参数传入是为了在钩子方法中调用agent_loop的工具提示和内容清理等辅助方法，虽然会循环引用，但这是必要的。
        # on_progress,是进度回调函数，主要用于在工具执行前发送工具提示和工具事件，让前端可以展示相关信息。
    ...
    async def on_stream(self, context: AgentHookContext, delta: str) -> None:
        ...
        # 进行增量清理，去掉<think>标签包裹的内容，确保流式更新时不会发送这些内容给前端（因为它们通常是模型的思考过程，不适合展示）。
    ...
    async def before_execute_tools(self, context: AgentHookContext) -> None:
        # 在工具执行前，如果有进度回调函数，先发送一次工具提示（如果有的话），以及即将执行的工具事件。这让前端可以在工具执行前展示相关提示和预期的工具调用。
        if self._on_progress:
            # # 场景A：如果当前不是流式输出，把模型在调用工具前说的话发给前端
        ...
        # 日志记录：它遍历了模型请求的所有工具，将工具的名称和传入的参数序列化为 JSON 并打印到系统日志中。 
...
class AgentLoop:
    ...
    It:
    1. Receives messages from the bus 接收消息
    2. Builds context with history, memory, skills 构建模型context,memory和技能
    3. Calls the LLM 调用模型生成响应和工具调用计划
    ...
    def _sync_subagent_runtime_limits(self) -> None:
        ...
        子代理的运行时限制（如最大迭代次数）需要与主循环的设置保持一致，以确保它们在执行过程中遵循相同的规则和限制。
        """
    ...
    def _register_default_tools(self) -> None:
        """
        登记默认工具：根据配置注册一系列默认工具，如文件操作、网络搜索、消息发送等。这些工具是模型在生成响应时可以调用的功能模块，提供了与外部环境交互的能力。
        """
        allowed_dir = (
    ...
    async def _connect_mcp(self) -> None:
        """
        连接mcp服务器：如果配置了MCP服务器，尝试连接它们以启用模型调用的中途插入功能。
        这是一个一次性的懒加载过程，只有在第一次运行循环时才会尝试连接。
        连接成功后，相关的工具调用可以在模型生成响应的过程中被动态插入，从而实现更灵活的交互和响应生成。
        """
    ...
    def _set_tool_context(
        ...
        """
        核心功能是：为系统中不同的“工具”（Tools）更新上下文路由信息。
        这样，当这些工具执行操作（如发送消息、创建定时任务、派生子任务）时，
        它们能准确知道要把结果发送到哪个平台（Channel）、哪个聊天会话（Chat ID）或哪个消息线程（Thread）。
        """
    ...
    async def _dispatch_command_inline(
        ...
        """
        VIP通道
        核心作用是：在主事件循环中直接（inline）执行一个命令，并将执行结果立刻发送出去，而不去排队或创建后台任务。
        """
    ...
    async def run(self) -> None:
        ...
        # loop循环不断地从消息总线（MessageBus）中消费入站消息（InboundMessage）。
        # 对于每条消息，首先尝试在1秒内获取，如果超时则进行一次自动压缩检查（auto_compact.check_expired），以清理过期的会话数据，
        # 然后继续等待下一条消息。
        while self._running:
            try:
                msg = await asyncio.wait_for(self.bus.consume_inbound(), timeout=1.0)
            except asyncio.TimeoutError:
                # 调用超时的情况下，检查并清理那些太久没说话的过期会话，释放内存资源，同时保持循环继续运行以等待新的消息。
                ...
                # 当系统收到关闭信号时（任务被 Cancel），它会鉴别这是不是真正的关机指令。如果是，就把异常抛出以平滑关闭；如果只是一些集成的杂音，就忽略它。
            ...
            # 对于非高优先级消息，首先计算出一个“有效的会话键”（effective session key），这个键用于确定消息应该被路由到哪个会话或线程中。
            effective_key = self._effective_session_key(msg)
            # 如果这个有效的会话键已经有一个正在运行的任务，那么这个消息就被放入一个“待处理队列”（pending queue）中，
            # 而不是立即创建一个新的任务来处理它。这种设计允许在当前任务完成之前，后续的消息能够被有序地处理，避免了并发冲突和资源竞争。
            if effective_key in self._pending_queues:
                # 如果这个消息是一个可调度的命令（比如用户在对话中输入了一个特定的指令），那么它会被直接处理，而不是放入待处理队列。
                # 系统命令就应该由系统直接消化。而不是丢给大模型思考
                ...
                # 把用户刚刚追加的新消息（pending_msg）瞬间塞进属于这个用户的专属队列里。
                try:
